...
除使用主定理外，还可以这样理解递归式的值：将递归过程看做一个二叉树。递归调用中的每一层的总代价均为 $c n$，其中 $c$ 为常数。而二叉树的层数应为 $log_2 n+1$，故整个算法的代价期望为 $Theta(n log_2 n)$。
...
对于 $n$ 阶矩阵乘法 $C = A dot B$，将三个矩阵都划分为四个部分，划分的四个部分仍遵守矩阵乘法法则。即：

$ mat(C_11, C_12; C_21, C_22) = mat(A_11, A_12; A_21, A_22) dot mat(B_11, B_12; B_21, B_22) $

对于 $C$ 矩阵中的第一部分，有：

$
  C_(11) = A_(11) dot B_(11) + A_(12) dot B_(21)
$
...
Strassen 方法的具体流程比较复杂。算法的效果是，将每一次调用的 8 次矩阵乘法替换为 7 次矩阵乘法和常数次的矩阵加法（复杂度为 $Theta(n^2)$）这也就意味着，递归树从每层 8 个分叉减少到了 7 个。随之，整个算法的复杂度降低到了 $Theta(n^(log_2 7)) approx Theta(n^(2.81))$。
...
具体过程在这里省略，也可以参照#link("http://www.cnblogs.com/zhoutaotao/p/3963048.html")[这篇文章]。
...
1. 若对某个常数 $epsilon$，有 $f(n) = O(n^(log_b a - epsilon))$，即 $f(n)$ 的代价比前项小，那么 $T(n) = Theta(n^(log_b a))$。
2. 若 $f(n) = O(n^(log_b a))$，那么 $T(n) = Theta(n^(log_b a) log_2 n)$。
3. 若对某个常数 $epsilon$，有 $f(n) = Omega(n^(log_b a + epsilon))$，即 $f(n)$ 的代价比前项大，那么 $T(n) = Theta(f(n))$。

简要地说，可以这样理解主定理：由递归带来的代价为 $Theta(n^(log_b a))$。

- 如果式中的常量更大，则常量决定了代价增长的速度。
- 反之，就是递归决定了函数增长的速度。
- 若二者相等，则需要乘上一个系数 $log_2 n$。
...
- 指示器变量：对于事件 $A$，指示器随机变量 $I(A)$ 在 $A$ 发生时为 1，否则为 0。如，抛硬币的随机指示器期望为 $1 / 2$。
...
显然，在最坏情况下，每一次面试都需要进行切换。这时，代价的期望为 $O(c_h n)$。其中，$c_h$ 是雇佣一个新的助理的代价。

=== 使用随机指示器分析雇用问题

显然，决定面试者是否被雇佣的随机指示器的期望 $E[X_i] = 1/i$。这意味着每个面试者有 $1/i$ 的概率比之前的所有人都好，因此被雇佣。那么总的雇佣人数的期望为：

$
  E[X] = E sum_(i=1)^n X_i = sum_(i=1)^n 1 / i = ln n + O(1)
$
...
Permute-by-sorting. 具体做法是：对于输入序列，生成一个相同长度的随机数的数组。为了让数组的内容足够唯一，让随机数取值在 $(1, n^3)$ 范围内。然后，在排序这个数组的同时，排序序列。这个做法的代价与比较排序的代价相等，为 $Theta(n log_2 n)$。
...
抛一枚硬币 $n$ 次，最长的连续正面的序列有多长？这个值的期望为 $Theta(log_2 n)$。可以从上界和下界两个方向来证明。
...
我们首先来尝试计算得到最好面试者的概率。设最好面试者在第 $i$ 个且被我们取到的事件为 $S_i$，考虑到前 $k$ 个面试者直接被我们排除掉，其中，当 $i<= k$ 时，不可能取到最佳面试者。于是，取得最好面试者的概率密度函数为：
...
$
  integral_k^n 1 / x dif x
  <= sum_(i=k)^(n - 1) 1 / i
  <= integral_(k - 1)^(i - 1) 1 / x dif x
$
...
以 $k$ 为未知数求导。当 $k = n / e$ 时，下界取得最大值。

因此，以此决定我们的 $k$ 取值，那么将以至少 $1 / e$ 的概率雇用到最好的面试者。
