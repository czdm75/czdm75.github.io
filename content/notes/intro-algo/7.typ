...
和其他各种树一样，B 树上操作的时间复杂度同样为 $O(log_t n)$，只是这里的 $t$ 比二叉树中的 2 通常要大得多。
...
#link("https://www.cs.usfca.edu/~galles/visualization/BTree.html")[这里]有一个不错的 B 树操作的可视化演示。
...
之前出现过的二叉堆在前四个操作上效果都不错，时间代价为 $O(log_2 n)$。然而，在合并堆的操作上，二叉堆的速度非常慢。可以这样实现：将两个堆直接合并起来，再执行建堆操作。这样，复杂度会达到 $Theta(n)$。斐波那契堆通常相比二叉堆具有更好的摊还代价。

#table(
  columns: 8,
  table.header([堆], [新建], [插入], [取堆顶], [删除堆顶], [合并堆], [减小], [删除]),
  [二叉],
  [$Theta(1)$],
  [$Theta(log_2 n)$],
  [$Theta(1)$],
...
另外，斐波那契堆中结点的度数，即子节点数将会得到限制。当支持堆合并操作时，最大度数 $D(n) <= floor(log_2 n)$，当支持后两个操作时，会使得 $D(n) = O(log_2 n)$。
...
#image("chap20_1.png", alt: "二叉树结构")

类似地，可以将其扩展，增加结点的大小：

#image("chap20_2.png", alt: "扩展的树索引")
...
#image("chap20_3.png", alt: "二叉树结构")

实际情况中，$sqrt(u)$ 不一定是整数。因此，使用 $sqrt^"↑" u > sqrt(u) > sqrt^"↓" u$，使得 $sqrt^"↑" u dot sqrt^"↓" u = u$。那么，`summary` 指向一棵 $"vEB"(sqrt^"↑" u)$，`cluster` 数组中的每一个指针指向 $"vEB"(sqrt^"↓" u)$。
