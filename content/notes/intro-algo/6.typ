...
这个问题的最优子结构是这样的：设 $X_m = (x_1, x_2, dots.c, x_m)$ 和 $Y_n = (y_1, y_2, dots.c, y_n)$ 是两个序列，其最长公共子序列为 $Z_k = (z_1, z_2, dots.c, z_k)$。

1. 若 $x_m=y_n$，则 $z_k~x_m~y_n$，$Z_k-1$ 是 $X_(m-1)$ 和 $Y_(n-1)$ 的最长公共子序列。
2. 若 $x_m!= y_n$，$z_k!= x_m$ 意味着 $Z$ 是 $X_(m-1)$ 和 $Y$ 的最长公共子序列。
3. 若 $x_m!= y_n$，$z_k!= y_n$ 意味着 $Z$ 是 $X$ 和 $Y_(n-1)$ 的最长公共子序列。
...
首先从递归动态规划方向来考虑问题。有 $n$ 个活动的集合 $(a_1, a_2, dots.c, a_n)$，并已经按照结束时间排序好。设 $c[i,j]$ 是最优解的大小，$S_(i j)$ 是所有在 $a_i$ 活动结束后开始，在 $a_j$ 活动开始前结束的活动的集合，即所有这两个活动之间的活动的集合，则：

$
  c[i,j] =
  cases(
    0 "if " S_(i j) = emptyset,
    max_(a_k in S_(i j)) (c[i, k] + c[k, j] + 1) "if " S_(i j) != emptyset
  )
$
...
$
  Phi(T) =
  cases(
    2 T."num" - T."size" "if " alpha >= 1 / 2,
    T."size" / 2 - T."num" "if " alpha < 1 / 2
  )
$
