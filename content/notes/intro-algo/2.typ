...
- 堆排序：$O(n log_2 n)$ 的原地排序算法

- 快速排序：期望为 $Theta(n log_2 n)$，最坏情况为 $Theta(n^2)$，实际应用中通常比堆排序快。同时，其常数系数很小，是排序大数组时的常用算法。

- 比较排序：通过对元素进行比较来决定，快排、归并、堆排序都是比较排序。比较排序的代价下界为 $Omega(n log_2 n)$。

- 线性时间排序：计数排序、基数排序、桶排序，在一定条件下，可以取得线性时间代价。

#table(
  columns: 3,
  table.header([算法], [最坏情况代价], [代价期望]),
  [插入排序], [$Theta(n^2)$], [$Theta(n^2)$],
  [归并排序], [$Theta(n log_2 n)$], [$Theta(n log_2 n)$],
  [堆排序], [$O(n log_2 n)$], [],
  [快速排序], [$Theta(n^2)$], [$Theta(n log_2 n)$],
...
复杂度为 $O(n log_2 n)$，常数个额外空间（原地排序）。

== 最大堆结构

最大堆是一个完全二叉树，且对于每一个结点，其子结点都比这个结点的值更小。通常使用数组来存储。这样，如果数组的下标从 1 开始，那么任意一个结点 $n$ 的左子结点为 $2n$，右子结点为 $2n+1$，父结点为 $floor(i/2)$（向下取整）。这样，我们可以轻松地利用移位指令来取得结点下标，获得比较高的性能。显然，二叉树的高度为 $Theta(log_2 n)$。于是，我们能得到一些堆上的基本操作的复杂度：

- 最大堆化（Max-heapify），复杂度为 $Theta(log_2 n)$。

- 构建最大堆（Build-max-heap），线性时间复杂度，将无序数据转化为最大堆。

- 堆排序（Heapsort），复杂度为 $O(n log_2 n)$，对一个数组进行原地排序。

- 插入（Max-Heap-Insert）、删除最大（Heap-Extract-Max）、增长 key（Heap-Increase-Key）、取得最大（Heap-Maximum），时间复杂度为 $O(log_2 n)$，功能是利用堆实现一个优先队列。
...
这个过程中的代价包括：判断和调整三个结点的过程，代价为 $Theta(1)$，加上递归调用的代价。每个子树的大小最多为 $(2 n) / 3$（当树的最底层半满时），那么我们可以用递归式 $T(n) <= T((2 n) / 3 + Theta(1))$ 来描述这个递归调用。使用主定理解得 $T(n) = O(log_2 n)$。也就是说，对于一个树高为 $h$ 的结点，最大堆化的时间复杂度为 $O(h)$。
...
$
  sum_(h=0)^(floor(log_2 n)) ceil(n / 2^(h + 1)) O(h)
  = O(n) sum_(h=0)^(floor(log_2 n)) h / (2^h)
$

又有公式：

$
  sum_(k=0)^(infinity) x^k = x / ((1-x)^2)
$

令 $x = 1 / 2$ 可以得到：

$
  sum_(k=0)^(infinity) h / (2^h)
  = (1 / 2) / ((1-1 / 2)^2) = 2
$
...
显然，取得最大元素的时间复杂度为 $Theta(1)$，删除最大元素的时间复杂度为 $O(log_2 n)$（相当于堆排序的第一步）。增大某个元素的 key，可以在修改之后将其不断与其父结点比较并交换，直到这个结点小于其父结点，意味着它来到了适合的位置上。这个过程的复杂度为 $O(log_2 n)$。在队列中增加结点，可以先在末尾加入一个 key 为 $-infinity$ 的结点（此时二叉树仍然是一个最大堆），再将其 key 增加为需要的值，并应用上面的算法。显然，复杂度为 $O(log_2 n)$。
...
可以证明，任何一种*常数*比例的划分，即使相当不均衡，都会得到 $Theta(n log_2 n)$ 的结果。

除此之外，我们还可以在划分中取随机的 pivot，并经过一次交换后再开始正常的流程。通过这种方式，可以在算法中引入随机性，避免最差情况的出现。

== 快速排序的概率分析

快速排序的运行时间取决于划分过程中进行比较的次数。定义一个随机指示器变量 $X_(i j) = I("compare" z_i "with" z_j)$，那么总的比较次数的期望为：

$
  E(X) = sum_(i=1)^(n - 1) sum_(j=i + 1)^n X_(i j)
  = sum_(i=1)^(n - 1) sum_(j=i + 1)^n E[X_(i j)]
  = sum_(i=1)^(n - 1) sum_(j=i + 1)^n Pr("compare" z_i "with" z_j)
$

考虑划分过程中的情况：所有元素都要与主元比较，且划分得到的两个数组之间不会发生比较。于是：

$
  Pr("compare" z_i "with" z_j)
  = Pr(z_i "is first pivot of set" Z_(i j)) + Pr(z_j "is first pivot of set" Z_(i j))
  = 2 / (j - i + 1)
$
...
$
  E(X) = sum_(i=1)^(n-1) sum_(j=i+1)^n 2 / (j - i + 1) = sum_(i=1)^(n-1) sum_(j=i+1)^n 2 / (k + 1) < sum_(i=1)^(n-1) sum_(j=i+1)^n 2 / k = O(n log_2 n)
$
...
之前介绍的归并排序、堆排序、快速排序都属于比较排序。比较排序在最坏情况下都为 $Omega(n log_2 n)$ 的。因此，复杂度为 $Theta(n log_2 n)$ 的归并排序和堆排序是渐进最优的，其他所有算法都只是在常数项上比它们更优。

一个比较排序可以抽象为一颗决策树。如，对于序列 $a,b$ 的决策树为，根节点判断两个元素的大小，左叶子节点是 $a<= b$ 的结果，右叶子节点是 $a>b$ 的结果。相应地，对于长度为 3 的序列，树将有三层，六个叶子节点。决策树的最大层数既是我们在得到结果之前必须做的比较的次数。
...
接下来讨论 “划分” 位的策略。如果把子排序中使用的位数从 1 位扩展到 $r$ 位，而 整个数字一共有 $b$ 位。那么，上式的 $d$ 就是 $b/r$，$k$ 就是 $2^r-1$。例如，对于一个 32 位的整数，我们将其分解为 4 个字节，于是 $b = 32$，$r = 8$，对每个字节进行计数排序。在相似的条件下，排序的复杂度就变成了 $Theta(b / r (n+2^r))$。

如果 $b < floor(log_2 n)$，那么对于任何 $r <= b$，都有 $(n+2^r) = Theta(n)$，算法的复杂度取决于 $b/r$。显然，当 $r = b$，复杂度取得最小值 $Theta(n)$。

如果 $b >= floor(log_2 n)$，对 $r$ 的值分情况来讨论。如果 $r = floor(log_2 n)$，可以得到最优时间代价 $Theta(b n / log_2 n)$。如果 $r < floor(log_2 n)$，值越小，$b/r$ 项的值就越大，$(n + 2^r)$ 仍然为 $Theta(n)$。反之，$r$ 越大，分子中的 $2^r$ 项比分母中的 $r$ 项增长得更快。因此，时间代价为 $Omega(b n / log_2 n)$。
...
$
  E[T(n)]
  = E[Theta(n) + sum_(i=0)^(n - 1) O(n_i^2)]
  = Theta(n) + sum_(i=0)^(n-1) O(E[n_i^2])
$

我们断言 $E[n_i^2] = 2 - 1 / n$，这个值可以利用随机指示器证明。最终，得到桶排序的时间复杂度：$Theta(n) + n dot O(2-1 / n) = Theta(n)$。

= 顺序统计量

== 顺序统计量

一个 $n$ 个数的集合的第 $i$ 个顺序统计量表示集合中第 $i$ 小的数。寻找顺序统计量的算法称为选择算法。显然，可以通过排序以 $O(n log_2 n)$ 的代价寻找到该统计量。

== 最大值和最小值

显然，最大值和最小值都属于顺序统计量的一种，可以通过代价为线性时间的一次扫描获得。一种同时取得最小值和最大值的方法是，对于每两个元素，先对这两个元素互相进行比较。随后，将较大者和最大值比较，较小者和最小值比较。这样，每两个元素只需要进行三次比较。比较次数从 $2n-2$ 次减少到 $3 / 2 n$ 次。
...
显然，分组、插入排序和划分都是线性代价的。递归寻找中位数的中位数这个过程的代价是 $T(n / 5)$，递归进行最后一步的代价最多是 $T(7 n / 10 + 6)$。于是有递归式：

$
  T(n) <=
  cases(
    O(1) "if " n<140,
    T(n / 5) + T(7 n / 10) + O(n) "if " n >= 140
  )
$
...
梳排序的想法有些类似于希尔排序：同样是使用一个步长，不过这个步长不用来划分出数组，而是仍然从头扫描，两两比较交换。如对于序列 (a, b, c, d, e)，取步长为 3，将会对 a 和 d 比较交换，再对 b 和 e 比较交换。缩小步长为 2，则对 (a, c)、(b, d)、(c, e) 比较和交换。因为每一轮不是完整的排序过程，所以步长只能以 1 为单位递减。梳排序的最差复杂度为 $O(n^2)$，期望为 $Theta(n log_2 n)$。

== 其他排序方法

除此之外，还可以使用二叉搜索树构造有序序列，这种结构非常适合插入和查找一定大小的值，复杂度为 $O(n log_2 n)$。选择排序是不断取顺序统计量，也就是不断从后部分序列中寻找最小者并放在头部的算法，复杂度为 $O(n^2)$。内省排序是快排的一种改进，当递归达到一定的深度之后改用堆排序，以兼有二者的优势，将最差代价控制在 $O(n log_2 n)$。
