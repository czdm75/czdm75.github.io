...
乘法散列对 $m$ 的值并不挑剔，一般取为一个 2 的幂，这样在计算机内部可以直接通过取一个数的高位来获得散列值。$A$ 的一个比较理想的值为 $sqrt(5) - 1 approx 0.618033 dots.c$。
...
定义散列表的装载因子，即元素数与槽数的比 $n / m$ 为 $alpha$。链表的查找时间取决于链表的长度，显然，最坏情况下整个散列表的查找时间为 $Theta(n)$。每个槽链表长度期望等于 $alpha$，时间代价期望为 $Theta(1+alpha)$。这是建立在散列完全均匀的假设下。
...
双重散列是开放寻址的最好方法之一，需要注意的是 $h_2(k)$ 必须与 $m$ 互质。一个方法是，取 $m$ 为 2 的幂，并设计一个永远产生奇数的 $h_2$。另一种方式是，取 $m$ 为质数，并让 $h_2(k)$ 略小于 $m$。前两种方法可能的探查序列有 $theta(m)$ 种，而双重散列将其提升到了 $Theta(m^2)$ 种。
...
$
  E[X]
  = sum_(i=1)^infinity Pr(X >= i)
  <= sum_(i=1)^infinity alpha^(i-1)
  = 1 / (1-alpha)
$
