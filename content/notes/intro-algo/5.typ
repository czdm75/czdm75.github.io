...
#link("https://upload.wikimedia.org/wikipedia/commons/thumb/b/be/Trie_example.svg/250px-Trie_example.svg.png")[Trie 树]

如果每一个分支不只保存一个位，将唯一子树与其父节点合并，就变成了基数树。基数树需要保存的路径信息变多了，但不再使用无用的结点。

#link("https://upload.wikimedia.org/wikipedia/commons/thumb/a/ae/Patricia_trie.svg/400px-Patricia_trie.svg.png")[基数树]

= Huffman 编码

Huffman 编码就是一种使用前缀的编码方法，其核心是前缀树的构建。Huffman 编码采用这样的基于概率的前缀构建：将所有可能的值作为叶子，并不断合并频率和最小的两个节点，最终构成一颗二叉前缀树。

#link("https://upload.wikimedia.org/wikipedia/commons/c/c8/Huffman_algorithm.gif")[Huffman 编码动图]
...
给定元素寻找秩的过程与之相反。从这个节点回溯到根，将所有的左子树 `size+1` 加起来，就是这个结点的秩。两种查找的复杂度均为 $O(log_2 n)$。

为了维护 `size` 域的值，在每一次插入和删除时，都需要回溯至根来修改，复杂度为 $O(log_2 n)$。此外，当发生旋转时，也要分别修改 `size`。修改的复杂度为 $O(1)$，所以插入和删除的复杂度不变，仍为 $O(log_2 n)$。
...
3. 对信息的维护：每个结点的 `max` 为 `max(x.int.high, x.left.max, x.right.max)`。于是，更新这个属性的复杂度为 $O(log_2 n)$。实际上，操作的复杂度只有 $O(1)$。
