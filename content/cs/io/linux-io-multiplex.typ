...
在 `select` 中，我们需要在所有 socket 的睡眠队列中插入对应的进程的标识。相比之下，`epoll` 将进程睡眠在 `ready_list` 上，而将操作 `ready_list` 的操作绑定在 socket 的睡眠队列上。这样，当 socket 发生事件时，所对应的事件就被加入到 `ready_list` 中。于是，在从阻塞中返回之前，我们只要遍历 `ready_list`，而不再需要遍历所有监听的事件。总的来看，`epoll` 把对于整个兴趣列表的遍历变成了对所有已经有事件就绪的兴趣的列表的遍历。
