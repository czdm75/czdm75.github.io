...
群是由一个集合与一个二元运算组成，满足四个性质：封闭性、结合律、单位元和逆元。以整数集 $bb(Z)$ 与加法运算组合起来的群为例：
...
范畴有三个条件：一个范畴由一系列物件 Object 构成的类 $"ob"(C)$ 和物件之间的态射组成的类 $"hom"(C)$ 构成。每一个态射是一个物件指向另一个物件的保持结构的一种关系。态射可以复合，就是说，如果对物件 $a,b,c$，有 $f: a arrow.r b, g: b arrow.r c$，那么存在 $g compose f: a arrow.r c$。一个范畴还满足两条公理：

- 这些态射满足结合律。$f compose (g compose  h)=(f compose  g) compose  h$。
- 范畴存在单位元。即，对每一个物件 $x$，都有 $f:x arrow.r  x$。
...
- 对每个态射 $forall f: X arrow.r Y in C$ 映射至 $F(f): F(X) arrow.r F(Y) in D$。
- $forall X in C$，很自然地，有 $F(bold("id")_X)=bold("id")_(F(X))$。
- $forall f: X arrow.r Y in C, g: Y arrow.r Z in C$，有 $F(g compose f)=F(g) compose F(f)$。
...
- $forall f: X arrow.r Y, F(f): F(Y) arrow.r F(X)$
- $forall f: X arrow.r Y in C, g: Y arrow.r Z in C, F(g compose f)=F(f) compose F(g)$
...
从 $C$ 到 $D$ 的函子范畴，记作 $[C, D]$，是以所有的协变函子 $F:C arrow.r  D$ 为对象，以函子之间的自然变换为态射的范畴。如果 $C,D$ 是同一个范畴，那么函子 $F$ 就是自函子，这个范畴也就是自函子范畴。

如果 $C,D$ 是范畴，$F,G$ 是二者之间的函子。那么 $forall X in C$，给出一个在 $D$ 的对象间的态射：$eta_X: F(X) arrow.r G(X)$，称为 $eta$ 在 $X$ 处的分量，使得 $forall f: X arrow.r Y in C$，有 $eta_Y compose F(f)=G(f) compose eta_X$。
...
那么，我们希望这个范畴具有什么样的性质，或者说我们希望这些态射是什么样的呢？在这里，对于函子 $M$，我们希望单位态射 $eta:1_H arrow.r  M$，（这里的 $1_H$ 表示 范畴中的物件 $H$ 的单位态射）态射的复合 $mu：M compose  M arrow.r  M$。符合这样性质的自函子就是单子 Monad。（由于单子是一个自函子，所以只需要一个态射及其复合即可定义它。）显然，这个复合操作完美地复合封闭性、结合律和单位元。因此，单子是一个自函子范畴上的幺半群。
...
这里的 `join` 就实现了我们上面提到的态射的复合：$M compose  M arrow.r  M$。也就是说，一个 `Option` 紧接着一个 `Option` 的结果仍然是一个 `Option`。因此，`Option` 是一个单子。
...
无论哪一种，都能够体现 $M compose  M arrow.r  M$。
